...
| 파일 | 역할 |
| --- | --- |
| `crawler.py` | 사람인 목록 페이지를 키워드 기반으로 수집해 SQLite 및 CSV에 저장 |
| `profile_builder.py` | 사용자가 입력한 직무/스킬/자기소개 문장에서 검색 키워드·경력·지역 정보를 추출 |
| `README.md` | 모듈 요약 및 명령 예시 |
| `기획안.md` | 전체 서비스 기획(데이터 흐름, RAG 구조, 일정) |
| `keyword_runner.py` | 프로필 입력을 받아 crawler까지 실행하는 헬퍼 |
| `vector_pipeline.py` | jobs.db → Chroma 벡터 DB 생성 |
| `rag_retriever.py` | dragonkue 임베딩 + BGE reranker retriever |
...
```bash
python career_matcher/profile_builder.py "3년차 백엔드인데 LLM 쪽 데이터 분석가로 전환하고 싶어요" --json
```
- 출력 필드
...
```
- 결과
  - SQLite `career_matcher/jobs.db`에 `job_postings` 테이블 upsert
  - CSV 백업 `career_matcher/csv/키워드_타임스탬프.csv`

※ 복수 프로필을 자동으로 순회하려면 `keyword_runner.py` 사용:
```bash
python career_matcher/keyword_runner.py --profile-file profiles.txt --pages 5 --delay 3
```

### STEP 3) 데이터 점검
```bash
sqlite3 career_matcher/jobs.db 'SELECT COUNT(*) FROM job_postings;'
sqlite3 career_matcher/jobs.db 'SELECT job_id,title,company FROM job_postings LIMIT 5;'
```
- 이후 단계: 전처리 → 임베딩 → 벡터DB 적재 → RAG 체인 연결(기획안 참조)

### STEP 4) 벡터 DB & Reranker 준비
```bash
python career_matcher/vector_pipeline.py --limit 1000
python career_matcher/rag_retriever.py  # 또는 import 후 build_reranked_retriever 사용
```

---

## 4. 추천 통합 플로우
1. 사용자 입력 수집 → `profile_builder.py`로 정규화
2. 필요한 키워드 리스트를 스케줄러에 전달 → `crawler.py` 실행
...
- **카탈로그 확장**: `profile_builder.py`에 정의된 직무/스킬/지역 카탈로그를 팀 도메인에 맞게 지속적으로 업데이트하세요.
